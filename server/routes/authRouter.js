...
import { authenticateToken, signToken, signGithubToken } from "../controllers/authentication.js";
...
authRouter.post("/guest", validateUser, async (req, res, next) => {
  try {
    const { handle, name, surname, email, password, profilePicUrl } = generateGuestCredentials();
    const existingUser = await queries.getUserByEmail(email);
    if (existingUser) {
      return res.status(409).json({ message: "Email already in use" });
    }
