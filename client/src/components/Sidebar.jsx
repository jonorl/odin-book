...
const Sidebar = ({ className = "" }) => { // Accept className prop with default empty string
  ...
  function odinBookButton() {
    navigate('/')
    currentPage === 1 ? window.location.reload() : setCurrentPage(1)
  }

  return (
    <div className={`p-4 relative ${
      // Use className prop if provided, otherwise use default responsive classes
      className || "w-16 sm:w-20 md:w-64 h-screen"
    } ${darkMode 
      ...
      <div className="mb-8 flex items-center justify-between">
        <h1 
          onClick={() => odinBookButton()} 
          className={`cursor-pointer text-2xl font-bold ${darkMode ? 'text-white' : 'text-blue-500'}`}
        >
          OdinBook
        </h1>
        <button
        ...
        <>
          <nav className="space-y-2">
            {menuItems.map((item, index) => {
              if (!item) return null; // Handle the conditional user item
          ...
          <div className="absolute left-4 right-4">
            ...
            >
              ...
              <MoreHorizontal 
                ref={moreHorizontalRef} 
                size={20} 
                className={`hidden md:block ${darkMode ? 'text-white' : 'text-black'}`} 
              />

              {userPopupMenu && (
                <div
                  ref={popupRef}
                  className={`absolute bottom-full mb-2 right-0 min-w-[200px] rounded-lg shadow-lg z-50 overflow-hidden ${
